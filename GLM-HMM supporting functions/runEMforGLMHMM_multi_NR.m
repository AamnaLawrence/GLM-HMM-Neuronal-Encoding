function [Ahat,What,logp,logpTrace,sqErr,jj,dlogp,gams,all_weights_iter] = runEMforGLMHMM_multi_NR(A0,W0,xdat,ydat,lambda, optsEM)
% EM run with M-step convergence using Newton-Raphson method
% Inputs:
% -------
    ...
    for neuron_id = 1:nNeurons 
        for iistate = 1:nStates
                ...
                lfunc = @(w) neglogli_Mstep_poissonGLM_ridge(w, ...
                              ...
                              gams(iistate,:)', ...
                              lambda(neuron_id));
                [w_opt, logp_trace] = newton_raphson(init_w, lfunc, 50, 1e-6);
                What{neuron_id}(:,iistate) = w_opt;
